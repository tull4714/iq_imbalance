...
down_cos=org_cos_sampling(X,Fq);
down_sin=org_sin_sampling(X,Fq);
...
%     rrx_ps(i)=real(ps(i))*(1/real(rx_ps(1)))+j*(imag(ps(i))*(1/imag(rx_ps(1))));

    if  real(ps(i))>=0 & imag(ps(i))>=0;
        rrx_ps(i)=real(ps(i))*(1/real(rx_ps(1)))+j*(imag(ps(i))*(1/imag(rx_ps(1))));
    elseif  real(ps(i))<0 & imag(ps(i))>=0;
        rrx_ps(i)=-real(ps(i))*(1/real(rx_ps(2)))+j*(imag(ps(i))*(1/imag(rx_ps(2))));
    elseif  real(ps(i))<=0 & imag(ps(i))<0;
        rrx_ps(i)=-real(ps(i))*(1/real(rx_ps(3)))-j*(imag(ps(i))*(1/imag(rx_ps(3))));
    else    real(ps(i))>0 & imag(ps(i))<0;
        rrx_ps(i)=real(ps(i))*(1/real(rx_ps(4)))-j*(imag(ps(i))*(1/imag(rx_ps(4))));
    end;
end;
I_r=real(rrx_ps);
Q_r=imag(rrx_ps);
up_cos_r=cos_sampling(X,Fq,C,upsilon);
up_sin_r=sin_sampling(X,C,Fq,upsilon);
conv_I_r=(I_r)'*up_cos_r;
conv_Iout_r=vec2mat(conv_I_r,1);
conv_Q_r=(Q_r)'*up_sin_r;
